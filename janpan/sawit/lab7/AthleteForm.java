...
public class AthleteForm extends MySimpleWindow {
    protected JPanel mainInfo_panel;// use protected for subclass to reuse.
    JPanel gender_panel;
    JLabel name, weight, height, birth, gender;
    JTextField name_feild, weight_feild, height_feild, birth_feild;// Declare instance attribute to use.
    ...
    public void addComponents() {
        ...
        this.name_feild = new JTextField(15);
        this.weight_feild = new JTextField(15);// in textfeild it has 15 gap at first output.
        this.height_feild = new JTextField(15);
        this.birth_feild = new JTextField(15);
        mainInfo_panel = new JPanel();
        mainInfo_panel.add(name);// panel add label.
        mainInfo_panel.add(name_feild);// panel add TextField.
        mainInfo_panel.add(weight);
        mainInfo_panel.add(weight_feild);
        mainInfo_panel.add(height);
        mainInfo_panel.add(height_feild);
        mainInfo_panel.add(birth);
        mainInfo_panel.add(birth_feild);
        mainInfo_panel.add(gender);
        male = new JRadioButton("Male");//JRadioButton in this program it has two gender to choose.
        female = new JRadioButton("Female");
        group = new ButtonGroup();//use for allow a user to choose only one radio button at a time.
        group.add(male);//add each gender to group.
        group.add(female);
